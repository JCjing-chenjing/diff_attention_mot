...
def parse_opt():
    ...
    parser.add_argument('--weights',  default='/Data/cj/test_redis/DanceTrack/diff_attention_mot/runs/train/exp2/best.pth', help='model path(s)')


    parser.add_argument('--batch_size', type=int, default=1, help='total batch size for all GPUs')   #32
    parser.add_argument('--num_workers', type=int, default=1, help='load data worker number')        #16
...
def main(args):
    ...
    test_dataset, test_loader = build_dataset(args, mode='val')
    
    if args.save_video:
        video_path=os.path.join(save_dir,'res.mp4')
        video = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc(*'MPEG'), 25, (1920, 1080))

    
    # for k, info in seq_info.items():
    for k, info in enumerate(test_loader):

      
        pre_imgs = info["pre_images"].to(device)            #data['pre_imgs'].to(device)  #torch.stack(data['pre_imgs'], 0).to(device)   #
        cur_images = info["cur_images"].to(device)          #data['cur_images'].to(device)
        # pre_target = pre_targets.to(device)               #data['pre_targets']
        # cur_target = cur_targets.to(device)               #data['cur_targets']
        pre_targets = convert_data2device(info["pre_targets"], args.data_type, device)
        cur_targets = convert_data2device(info["cur_targets"], args.data_type, device)
        ...
        if args.save_video or args.save_img or args.show_video:
            ...
            img = cv2.imread(info['cur_targets']["img_path"])
            h,w,_ = img.shape
            box_lst[:,::2] = box_lst[:,::2]*w
            ...
            if args.save_img:
                save_img_path=os.path.join(save_dir)
                build_dir(save_img_path)
                img_name = os.path.basename(info['cur_targets']["img_path"]).split(".")[0] + "_" + str(k) + ".jpg"
